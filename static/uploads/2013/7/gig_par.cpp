...
/*
#define WHILE_ERROR_TESTING

#ifdef WHILE_ERROR_TESTING
#include "mex.h"
#define WHILE_ERROR_OUTPUT {mexPrintf("  alpha: %e\n", alpha);  \
  mexPrintf("   beta: %e\n", beta);                             \
  mexPrintf("  p_abs: %e\n", p_abs);                            \
  mexPrintf("      p: %e\n", p);                                \
  mexPrintf("      a: %e\n", alpha*beta);                       \
...
//we also need an upper limit for rejection-sampling (using 100'000)
const unsigned int reject_max_it = 100000;
*/
...
double gig::sqrt_gig_ratio(double x_in, double m_in){
	...
	return ret;
  //	return exp( (p_abs - 1)/2 * (log(x_in) - log(m_in)) +
  //              ( m_in + 1/m_in - x_in - 1/x_in  )/(2 * two_d_beta ) );
}	
...
double gig::sample(double p_in, double a_in, double b_in){
